...
**Based on the official OpenAI Cookbook examples.**
...
3. Add these variables (masked/protected as appropriate):
   - `OPENAI_API_KEY`
   - `GITLAB_TOKEN`
      ...
      steps {
        withCredentials([
          ...
          sh '''
            ...
            if command -v codex >/dev/null 2>&1; then
              printenv OPENAI_API_KEY | codex login --with-api-key && \
              codex exec --output-schema "${REVIEW_SCHEMA}" \
                         --output-last-message "${REVIEW_OUTPUT}" \
                         --sandbox read-only \
                         - < "${REVIEW_PROMPT}"
            else
              echo "Codex CLI not found; creating a stub output for demo continuity."
...
|---------|----------------|--------------|---------|
| **Action Available** | Yes (official) | Via CLI | Via CLI |
| **Setup** | Pre-installed runner | Manual CLI install | Manual CLI install |
| **Secrets** | Built-in | CI/CD Variables | Credentials |
| **Concurrency Control** | Built-in | Manual via rules | `disableConcurrentBuilds()` |
| **Official Example** | Yes | Yes | Yes |

---

## References

- [Official Cookbook - Build Code Review with Codex SDK](https://developers.openai.com/cookbook/examples/codex/build_code_review_with_codex_sdk)
